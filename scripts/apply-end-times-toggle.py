...
# Engine: let both quick and intensive cheese searches exclude End Times.
...
# UI / persistence / requests.
p = Path('index.html')
t = p.read_text()
t = replace_once(
    t,
    ".tower-pool-warning{margin-top:7px;color:#6e7d8f;font-size:7px;line-height:1.4}@media(max-width:640px){.tower-pool-suggestions{grid-template-columns:1fr}}\n",
    ".tower-pool-warning{margin-top:7px;color:#6e7d8f;font-size:7px;line-height:1.4}.tower-pool-options{display:flex;align-items:center;gap:7px;flex-wrap:wrap;justify-content:flex-end}.tower-endtimes-owned{border:1px solid #355e5a;border-radius:999px;background:rgba(135,216,202,.07);color:#c8e8e2;padding:5px 8px;font-size:8px;font-weight:750;cursor:pointer}.tower-endtimes-owned.off{border-color:#493039;background:#160e11;color:#b88a93}.tower-endtimes-owned:hover{filter:brightness(1.12)}@media(max-width:640px){.tower-pool-suggestions{grid-template-columns:1fr}.tower-pool-options{justify-content:flex-start}}\n",
...
t = replace_once(
    t,
    "<div class=\"tower-pool-head\"><div><b>Search card pool</b><small>Click a default card you do not own to disable it. Add any other card you want the search to try.</small></div><span>${activePool.length} active cards</span></div>",
    "<div class=\"tower-pool-head\"><div><b>Search card pool</b><small>Click a default card you do not own to disable it. Add any other card you want the search to try.</small></div><div class=\"tower-pool-options\"><span>${activePool.length} active cards</span><button type=\"button\" class=\"tower-endtimes-owned ${state.towerHasEndTimes?'':'off'}\" data-tower-end-times>${state.towerHasEndTimes?'End Times · OWNED':'End Times · NOT OWNED'}</button></div></div>",
    'End Times pool toggle UI',
)
t = replace_once(
    t,
...
t = replace_once(
    t,
    "seed:state.seed,towerExcludedCards:state.towerExcludedCards,towerAddedCards:state.towerAddedCards}))",
    "seed:state.seed,towerExcludedCards:state.towerExcludedCards,towerAddedCards:state.towerAddedCards,towerHasEndTimes:state.towerHasEndTimes}))",
    'persist End Times ownership',
)
t = replace_once(
    t,
    "state.towerAddedCards=Array.isArray(s.towerAddedCards)?[...new Set(s.towerAddedCards.map(String))].filter(name=>{const card=cardByName(name);return Boolean(card&&!card.unobtainable&&!TOWER_DEFAULT_CHEESE_CARDS.includes(name))}):[];}catch(_){}",
    "state.towerAddedCards=Array.isArray(s.towerAddedCards)?[...new Set(s.towerAddedCards.map(String))].filter(name=>{const card=cardByName(name);return Boolean(card&&!card.unobtainable&&!TOWER_DEFAULT_CHEESE_CARDS.includes(name))}):[];state.towerHasEndTimes=s.towerHasEndTimes!==false;}catch(_){}",
    'restore End Times ownership',
)
t = replace_once(
    t,
    "intensive:true,excludedCards:[...state.towerExcludedCards],addedCards:[...state.towerAddedCards],shardIndex,shardCount:workerCount});\n",
...
t = replace_once(
    t,
    "intensive:false,excludedCards:[...state.towerExcludedCards],addedCards:[...state.towerAddedCards]});\n",
...
# Regression coverage: disabling End Times should remove its four border variants.
