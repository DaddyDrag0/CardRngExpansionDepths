...
new = """  const overCapBans = eligibleNames.slice(0, MAX_DEPTH_BANS + 1)
  ...
  for (const name of ['Samurai', 'Seraphim', 'Loki', 'Fuxi', 'Parallax', 'Nán Fāng Zhū Què', 'Brachiosaurus', 'Jersey Devil']) {
    assert(isDepthsSourceEligible(cardByName(name)), `Newly unbanned Depth card ${name} must be source-eligible`)
  }"""
if old not in text:
    raise SystemExit('Depth-ban regression block not found')
