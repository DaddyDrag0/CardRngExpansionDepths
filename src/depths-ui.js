...
function decodeTeam(code){
  ...
  const hardCap=10000;
  const statAurasPerDay=r.estimatedSecondsMedian>0?r.auraPackMedian*(86400/r.estimatedSecondsMedian):0;
  const hardMetrics=`<div><span>Estimated Hard Depth range</span><b>${full(r.estimatedFloorLow)} – ${full(r.estimatedFloorHigh)}</b></div><div><span>Median Hard Depth</span><b>${one(r.medianFloor)}</b></div><div title="${full(r.auraPackLow)} – ${full(r.auraPackHigh)} Stat Aura Packs"><span>Stat Aura Pack reward</span><b>${compact(r.auraPackLow)} – ${compact(r.auraPackHigh)}</b><small>Median: ${compact(r.auraPackMedian)} · 3× normal Aura Pack gain</small></div><div><span>Stat Auras / day</span><b>≈${compact(statAurasPerDay)}</b><small>Using median Stat Aura Pack reward and estimated clear time</small></div><div><span>Weather Token scaling</span><b>No value</b><small>Exact rate not confirmed yet · scaling stops at Hard floor ${full(hardCap)}</small></div><div><span>Estimated clear time</span><b>${duration(r.estimatedSecondsMedian)}</b><small>${duration(r.estimatedSecondsLow)}–${duration(r.estimatedSecondsHigh)} · Hard Mode · ≈${compact(r.auraCardsPerHour)} Stat Aura Packs/hour</small></div>`;
