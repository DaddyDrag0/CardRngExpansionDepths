...
function resultCard(team,index){
  ...
  const hardMetrics=`<div><span>Estimated Hard Depth range</span><b>${full(r.estimatedFloorLow)} – ${full(r.estimatedFloorHigh)}</b></div><div><span>Median Hard Depth</span><b>${one(r.medianFloor)}</b></div><div><span>Normal-stat equivalent</span><b>${full(r.estimatedFloorLow*10)} – ${full(r.estimatedFloorHigh*10)}</b><small>Hard enemy stats use floor ×10</small></div><div><span>Weather Token scaling</span><b>${hardProgress>=100?'CAPPED':one(hardProgress)+'%'}</b><small>Stops improving at Hard floor ${full(hardCap)}</small></div><div><span>Estimated clear time</span><b>${duration(r.estimatedSecondsMedian)}</b><small>${duration(r.estimatedSecondsLow)}–${duration(r.estimatedSecondsHigh)} · Hard Mode</small></div>`;
