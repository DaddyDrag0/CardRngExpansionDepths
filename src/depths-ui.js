(() => {
  ...
  const state={cards:[],auras:[],playerStatAuras:[],hardDepthRewards:null,weatherEffects:{},abilities:{},thumbs:{},teams:Array.from({length:5},blankTeam),activeTeam:0,activeSlot:0,query:'',depthBanLayouts:initialDepthBanLayouts,activeDepthBanLayout:0,depthBans:initialDepthBanLayouts[0],depthBanQuery:'',bountifulDepths:false,chronoShard:true,hardMode:false,rebanLegacyDepths:false,battleSpeedStructureLevel:0,skillTreeBattleSpeedLevel:0,runs:15,startFloor:1,cap:150000,seed:1000,running:false,runningLabel:'',workerReady:false,lastProgressRender:0,view:'depths',towerFloor:105,towerEnemies:['','','',''],towerResult:null,towerDifficulty:'Impossible',towerRuns:10000,towerSim:null,towerSimRunning:false,towerSimLabel:'',towerOverrides:['','','',''],towerBorders:Array.from({length:4},()=>[]),towerMutations:Array.from({length:4},()=>''),towerAbilityAura:'',towerAbilityAuraBorder:'',towerSearch:null,towerSearchRunning:false,towerSearchLabel:'',towerSearchMode:'quick',towerExcludedCards:['Robin Hood','Pandora'],towerAddedCards:[],towerPoolQuery:'',towerHasEndTimes:false};
  const SNAPSHOT_PARAM='snapshot';
  let reportSnapshotMode=false;
  const reportEndpoint=()=>String(window.__CRX_FEEDBACK_ENDPOINT__||document.querySelector('meta[name="crx-feedback-endpoint"]')?.content||'').trim();
  const cloneJson=value=>value==null?value:JSON.parse(JSON.stringify(value));
  function syncViewUrl(view){
    const url=new URL(location.href);
    if(view==='tower')url.searchParams.set('view','tower');
  ...
  const TOWER_DEFAULT_CHEESE_CARDS=['Judgment Day','Robin Hood','Parallax','Piccolo','Pandora','Kuchisake-onna','Fate Seamstress','Kira',"Hell's Army",'Noveau Riche'];
  const TOWER_DISABLED_CHEESE_CARDS=new Set(['True Prophet','Ice King','Surtr','Control Freak']);
  ...
  function bindTowerEvents(){
    root.querySelectorAll('[data-tower-back-team]').forEach(el=>el.addEventListener('click',()=>{state.view='depths';syncViewUrl('depths');state.activeTeam=Number(el.dataset.towerBackTeam);state.activeSlot=0;render()}));
...
function decodeTeam(code){
  ...
  function hardRewardsPanel(){
  const data=state.hardDepthRewards;
  if(!data)return '';
  const rewards=(data.rewards||[]).map(item=>`<div class="hard-reward-item"><span>${esc(item.name)}</span><b>${esc(item.rarityLabel||'')}</b>${item.notes?`<small>${esc(item.notes)}</small>`:''}</div>`).join('');
  const statAuras=(state.playerStatAuras||[]).map(aura=>`<div class="hard-stat-aura">${portrait(aura,'hard-stat-aura-icon')}<div><b>${esc(aura.name)}</b><span>1 / ${rarityCompact(aura.rarity)}</span><small>${esc(aura.description.replace('STAT',`${aura.base}–${aura.max}`))}</small></div></div>`).join('');
  return `<div class="hard-reward-preview">
    <div class="hard-reward-head"><div><span class="kicker">HARD DEPTHS REWARDS</span><h4>Separate Hard Mode reward table</h4></div><i>Token scaling caps at floor ${full(data.weatherTokenScalingCapFloor||10000)}</i></div>
    <div class="hard-reward-grid">${rewards}</div>
    ...
    <div class="hard-reward-source-note">Stat Aura Packs are verified at 3× the normal Depth Aura Pack gain (Hard depth 1,429 = 298,842 packs, matching 3 × 99,614). Exact Weather Token and potion drop formulas are still server-side, so those odds are not estimated yet.</div>
  </div>`
}
...
function resultCard(team,index){
  ...
  const hard=Boolean(r.hardMode??state.hardMode);
  const drops=hard?null:r.potionRewards;
  ...
  const normalMetrics=`<div><span>Estimated Depth range</span><b>${full(r.estimatedFloorLow)} – ${full(r.estimatedFloorHigh)}</b></div><div><span>Median Depth</span><b>${one(r.medianFloor)}</b></div><div title="${full(r.auraPackLow)} – ${full(r.auraPackHigh)} Aura Packs"><span>Aura Pack reward</span><b>${compact(r.auraPackLow)} – ${compact(r.auraPackHigh)}</b><small>Median: ${compact(r.auraPackMedian)} packs</small></div><div title="One Aura Pack opens one Aura Card roll"><span>Aura cards</span><b>≈${compact(r.auraPackMedian)}</b><small>${compact(r.auraPackLow)}–${compact(r.auraPackHigh)} across the range · 1 card / pack</small></div><div><span>Estimated clear time</span><b>${duration(r.estimatedSecondsMedian)}</b><small>${duration(r.estimatedSecondsLow)}–${duration(r.estimatedSecondsHigh)} · Battle Speed 3${state.chronoShard?' + Chrono Shard':''} + Structure L${state.battleSpeedStructureLevel} + Skill Tree L${state.skillTreeBattleSpeedLevel} + floor scaling</small></div><div><span>Aura cards / hour</span><b>≈${compact(r.auraCardsPerHour)}</b><small>Using the median reward and estimated median clear time</small></div>`;

  const hardCap=Number(state.hardDepthRewards?.weatherTokenScalingCapFloor||10000);
  const hardProgress=Math.min(100,Math.max(0,(Number(r.medianFloor)||0)/hardCap*100));
  const hardMetrics=`<div><span>Estimated Hard Depth range</span><b>${full(r.estimatedFloorLow)} – ${full(r.estimatedFloorHigh)}</b></div><div><span>Median Hard Depth</span><b>${one(r.medianFloor)}</b></div><div title="${full(r.auraPackLow)} – ${full(r.auraPackHigh)} Stat Aura Packs"><span>Stat Aura Pack reward</span><b>${compact(r.auraPackLow)} – ${compact(r.auraPackHigh)}</b><small>Median: ${compact(r.auraPackMedian)} · 3× normal Aura Pack gain</small></div><div><span>Normal-stat equivalent</span><b>${full(r.estimatedFloorLow*10)} – ${full(r.estimatedFloorHigh*10)}</b><small>Hard enemy stats use floor ×10</small></div><div><span>Weather Token scaling</span><b>${hardProgress>=100?'CAPPED':one(hardProgress)+'%'}</b><small>Stops improving at Hard floor ${full(hardCap)}</small></div><div><span>Estimated clear time</span><b>${duration(r.estimatedSecondsMedian)}</b><small>${duration(r.estimatedSecondsLow)}–${duration(r.estimatedSecondsHigh)} · Hard Mode · ≈${compact(r.auraCardsPerHour)} Stat Aura Packs/hour</small></div>`;

  return `<section class="result-card ${index===state.activeTeam?'active-result':''}"><div class="result-head"><div><span class="kicker">TEAM ${index+1}${hard?' · HARD':''}</span></div></div><div class="result-metrics ${hard?'hard-result-metrics':''}">${hard?hardMetrics:normalMetrics}</div>${dropHtml}<div class="result-meta"><span>${r.runs.length} runs</span><span>${one(avgTurns)} avg turns</span><span>${team.elapsedMs<1000?`${Math.round(team.elapsedMs)} ms`:`${(team.elapsedMs/1000).toFixed(2)} s`} compute</span></div><div class="floor-strip">${r.runs.map((run,i)=>`<span role="button" tabindex="0" style="cursor:pointer" data-debug-team="${index}" data-debug-run="${i}" title="Run ${i+1}${run.turnLimitEnemy?' · TURN LIMIT vs '+run.turnLimitEnemy:''}${run.endingEnemies?.length?' · '+run.endingEnemies.join(' / '):''} · click for debug">${full(run.deathFloor)}</span>`).join('')}</div>${endingHtml}${turnLimitHtml}${r.unsupportedAbilities?.length?`<div class="result-warning">Unsupported: ${esc(r.unsupportedAbilities.join(', '))}</div>`:''}</section>`
}
  ...
  function buildReportSnapshot(){
    return {
      ...
      viewport:{scrollX:Math.round(window.scrollX||0),scrollY:Math.round(window.scrollY||0),width:window.innerWidth,height:window.innerHeight},
      runtime:{running:Boolean(state.running),runningLabel:String(state.runningLabel||''),towerSimRunning:Boolean(state.towerSimRunning),towerSimLabel:String(state.towerSimLabel||''),towerSearchRunning:Boolean(state.towerSearchRunning),towerSearchLabel:String(state.towerSearchLabel||'')},
      state:{
        teams:state.teams.map(team=>({
          cards:team.cards.map(slot=>({cardName:slot.cardName,borders:[...(slot.borders||[])],mutationWeather:slot.mutationWeather||''})),
          statAura:team.statAura||'',statAuraBorder:team.statAuraBorder||'',abilityAura:team.abilityAura||'',abilityAuraBorder:team.abilityAuraBorder||'',
          result:cloneJson(team.result),elapsedMs:Number(team.elapsedMs)||0,lastError:String(team.lastError||''),lastSeed:Number(team.lastSeed)||0,
        })),
        activeTeam:state.activeTeam,activeSlot:state.activeSlot,query:state.query,
        ...
        bountifulDepths:state.bountifulDepths,chronoShard:state.chronoShard,hardMode:state.hardMode,rebanLegacyDepths:state.rebanLegacyDepths,
        ...
        towerExcludedCards:[...state.towerExcludedCards],towerAddedCards:[...state.towerAddedCards],towerPoolQuery:state.towerPoolQuery,towerHasEndTimes:state.towerHasEndTimes,
      },
    };
  }
  window.__CRX_CREATE_REPORT_SNAPSHOT__=buildReportSnapshot;

  function hydrateSnapshotTeam(src){
    const team=blankTeam();
    if(!src||typeof src!=='object')return team;
    team.cards=Array.from({length:4},(_,index)=>{
      const slot=src.cards?.[index]||{},name=String(slot.cardName||'');
      return {cardName:name&&cardByName(name)?name:'',borders:Array.isArray(slot.borders)?slot.borders.filter(border=>CARD_BORDERS.includes(border)):[],mutationWeather:MUTATION_WEATHERS.includes(slot.mutationWeather)?slot.mutationWeather:''};
    });
    team.statAura=src.statAura&&auraByName(String(src.statAura))?String(src.statAura):'';
    team.statAuraBorder=AURA_BORDERS.includes(src.statAuraBorder)?src.statAuraBorder:'';
    team.abilityAura=src.abilityAura&&auraByName(String(src.abilityAura))?String(src.abilityAura):'';
    team.abilityAuraBorder=AURA_BORDERS.includes(src.abilityAuraBorder)?src.abilityAuraBorder:'';
    team.result=src.result&&typeof src.result==='object'?cloneJson(src.result):null;
    team.elapsedMs=Math.max(0,Number(src.elapsedMs)||0);
    team.lastError=String(src.lastError||'').slice(0,1000);
    team.lastSeed=Number(src.lastSeed)||0;
    return team;
  }

  function applyReportSnapshot(snapshot){
    if(!snapshot||snapshot.v!==1||!snapshot.state||typeof snapshot.state!=='object')throw new Error('Unsupported report snapshot');
    const s=snapshot.state;
    state.teams=Array.from({length:5},(_,index)=>hydrateSnapshotTeam(s.teams?.[index]));
    state.activeTeam=Math.max(0,Math.min(4,Math.floor(Number(s.activeTeam)||0)));
    state.activeSlot=Math.max(0,Math.min(3,Math.floor(Number(s.activeSlot)||0)));
    state.query=String(s.query||'').slice(0,200);
    state.depthBanLayouts=Array.isArray(s.depthBanLayouts)?Array.from({length:4},(_,index)=>sanitizeBanList(s.depthBanLayouts[index])):blankBanLayouts();
    state.activeDepthBanLayout=Math.max(0,Math.min(3,Math.floor(Number(s.activeDepthBanLayout)||0)));
    state.depthBans=state.depthBanLayouts[state.activeDepthBanLayout];
    state.depthBanQuery=String(s.depthBanQuery||'').slice(0,200);
    state.bountifulDepths=Boolean(s.bountifulDepths);
    state.chronoShard=s.chronoShard!==false;
    state.rebanLegacyDepths=Boolean(s.rebanLegacyDepths);
    state.battleSpeedStructureLevel=Math.max(0,Math.min(7,Math.floor(Number(s.battleSpeedStructureLevel)||0)));
    state.skillTreeBattleSpeedLevel=Math.max(0,Math.min(4,Math.floor(Number(s.skillTreeBattleSpeedLevel)||0)));
    state.runs=[1,3,8,15,30,50].includes(Number(s.runs))?Number(s.runs):15;
    state.startFloor=Math.min(80000,Math.max(1,Math.floor(Number(s.startFloor)||1)));
    state.cap=150000;
    state.seed=Number(s.seed)||1000;
    state.view=s.view==='tower'?'tower':'depths';
    state.towerFloor=Math.min(105,Math.max(1,Math.floor(Number(s.towerFloor)||105)));
    state.towerEnemies=Array.from({length:4},(_,index)=>{const name=String(s.towerEnemies?.[index]||'');return name&&cardByName(name)?name:''});
    state.towerResult=s.towerResult&&typeof s.towerResult==='object'?cloneJson(s.towerResult):null;
    state.towerDifficulty=['Normal','Hard','Extreme','Hell','Impossible'].includes(s.towerDifficulty)?s.towerDifficulty:'Impossible';
    state.towerRuns=Math.max(1,Math.min(100000,Math.floor(Number(s.towerRuns)||10000)));
    state.towerSim=s.towerSim&&typeof s.towerSim==='object'?cloneJson(s.towerSim):null;
    state.towerOverrides=Array.from({length:4},(_,index)=>{const name=String(s.towerOverrides?.[index]||'');return name&&cardByName(name)?name:''});
    state.towerBorders=Array.from({length:4},(_,index)=>Array.isArray(s.towerBorders?.[index])?s.towerBorders[index].filter(border=>CARD_BORDERS.includes(border)):[]);
    state.towerMutations=Array.from({length:4},(_,index)=>MUTATION_WEATHERS.includes(s.towerMutations?.[index])?s.towerMutations[index]:'');
    state.towerAbilityAura=s.towerAbilityAura&&auraByName(String(s.towerAbilityAura))?String(s.towerAbilityAura):'';
    state.towerAbilityAuraBorder=AURA_BORDERS.includes(s.towerAbilityAuraBorder)?s.towerAbilityAuraBorder:'';
    state.towerSearch=s.towerSearch&&typeof s.towerSearch==='object'?cloneJson(s.towerSearch):null;
    state.towerSearchMode=s.towerSearchMode==='intensive'?'intensive':'quick';
    state.towerExcludedCards=Array.isArray(s.towerExcludedCards)?[...new Set(s.towerExcludedCards.map(String))].filter(name=>TOWER_DEFAULT_CHEESE_CARDS.includes(name)):[];
    state.towerAddedCards=Array.isArray(s.towerAddedCards)?[...new Set(s.towerAddedCards.map(String))].filter(name=>{const card=cardByName(name);return Boolean(card&&!card.unobtainable&&!TOWER_DEFAULT_CHEESE_CARDS.includes(name))}):[];
    state.towerPoolQuery=String(s.towerPoolQuery||'').slice(0,200);
    state.towerHasEndTimes=s.towerHasEndTimes!==false;
    state.running=false;state.runningLabel='';state.towerSimRunning=false;state.towerSimLabel='';state.towerSearchRunning=false;state.towerSearchLabel='';
    reportSnapshotMode=true;
    const theme=String(snapshot.theme||'');
    if(['default','scarlet','slate','lava'].includes(theme)){
...
function bindTooltips(){const show=el=>{const card=cardByName(el.dataset.tooltipCard);if(!card)return;const desc=state.abilities[card.ability]||'No ability description found.',slotIndex=Number(el.dataset.tooltipSlot),slot=Number.isInteger(slotIndex)&&slotIndex>=0?current().cards[slotIndex]:null,borders=slot?.cardName===card.name&&Array.isArray(slot.borders)?slot.borders:[],mutation=slot?.cardName===card.name&&MUTATION_WEATHERS.includes(slot?.mutationWeather)?slot.mutationWeather:'',rarity=rarityWithBorders(card,borders),rarityLabel=borders.length?'Modified rarity':'Base rarity';tooltip.innerHTML=`<div class="tip-name">${esc(card.name)}</div><div class="tip-ability">${esc(card.ability||'No ability')}</div><div class="tip-desc">${esc(desc)}</div><div class="tip-rarity">${rarityLabel}: 1 / ${rarityCompact(rarity||0)}${mutation?` · ${esc(mutation)} Mutation ×${MUTATION_MULT[mutation]}`:''}</div>`;tooltip.classList.add('show');const r=el.getBoundingClientRect(),w=Math.min(320,window.innerWidth-24);let left=r.right+10;if(left+w>window.innerWidth-12)left=Math.max(12,r.left-w-10);tooltip.style.left=`${left}px`;tooltip.style.top=`${Math.min(window.innerHeight-180,Math.max(12,r.top))}px`};const hide=()=>tooltip.classList.remove('show');root.querySelectorAll('[data-tooltip-card]').forEach(el=>{el.addEventListener('mouseenter',()=>show(el));el.addEventListener('mouseleave',hide);el.addEventListener('focus',()=>show(el));el.addEventListener('blur',hide)})}
  ...
  function runTowerCheeseSearch(mode='quick'){
    ...
    state.towerResult=null;state.towerSim=null;state.towerSearchMode=mode==='intensive'?'intensive':'quick';state.towerSearch={progress:{phase:state.towerSearchMode==='intensive'?'exhaustive':'quick',completed:0,total:0,battleSimulations:0}};state.towerSearchRunning=true;state.towerSearchLabel=state.towerSearchMode==='intensive'?'Starting Deep Search…':'Starting quick search…';
  ...
  function askWorker(index,batchSeed){if(!worker||!state.workerReady)return Promise.reject(new Error('Simulation worker is unavailable. Refresh and try again.'));const id=++requestId,t=state.teams[index];return new Promise((resolve,reject)=>{pending.set(id,{resolve,reject,teamIndex:index});worker.postMessage({id,loadout:loadoutFor(t),runs:state.runs,startFloor:state.startFloor,floorCap:state.cap,seed:batchSeed>>>0,bannedCardNames:[...state.depthBans],bountifulDepths:state.bountifulDepths,chronoShard:state.chronoShard,hardMode:state.hardMode,battleSpeedStructureLevel:state.battleSpeedStructureLevel,skillTreeBattleSpeedLevel:state.skillTreeBattleSpeedLevel})})}
  ...
  async function liveThumbs(){const missing=[...new Set([...state.cards,...state.auras,...state.playerStatAuras].map(x=>x.imageAssetId).filter(id=>id&&!state.thumbs[String(id)]).map(String))];if(!missing.length)return;try{for(let i=0;i<missing.length;i+=20){const chunk=missing.slice(i,i+20),p=new URLSearchParams({assetIds:chunk.join(','),size:'420x420',format:'Png',isCircular:'false'}),r=await fetch(`https://thumbnails.roblox.com/v1/assets?${p}`,{credentials:'omit'});if(!r.ok)continue;const j=await r.json();for(const item of j.data||[])if(item.imageUrl)state.thumbs[String(item.targetId)]=item.imageUrl}render()}catch(_){}}
  async function load(){try{const cardFiles=[1,2,3,4,5,6,7].map(i=>fetch(versioned(`./src/data/cards-${i}.json`),{cache:'no-store'}).then(r=>r.json())),auraFiles=[1,2].map(i=>fetch(versioned(`./src/data/auras-${i}.json`),{cache:'no-store'}).then(r=>r.json())),abilityFiles=[1,2,3,4].map(i=>fetch(versioned(`./src/data/abilities-${i}.json`),{cache:'no-store'}).then(r=>r.json())),playerStatPromise=fetch(versioned('./src/data/player-stat-auras.json'),{cache:'no-store'}).then(r=>r.ok?r.json():[]).catch(()=>[]),hardRewardsPromise=fetch(versioned('./src/data/hard-depths-rewards.json'),{cache:'no-store'}).then(r=>r.ok?r.json():null).catch(()=>null),weatherEffectsPromise=fetch(versioned('./src/data/weather-effects.json'),{cache:'no-store'}).then(r=>r.ok?r.json():{}).catch(()=>({})),thumbPromise=fetch(versioned('./src/data/thumbnails.json'),{cache:'no-store'}).then(r=>r.ok?r.json():{}).catch(()=>({}));const[cardSets,auraSets,abilitySets,playerStatAuras,hardDepthRewards,weatherEffects,thumbs]=await Promise.all([Promise.all(cardFiles),Promise.all(auraFiles),Promise.all(abilityFiles),playerStatPromise,hardRewardsPromise,weatherEffectsPromise,thumbPromise]);state.cards=cardSets.flat();state.auras=auraSets.flat();state.playerStatAuras=Array.isArray(playerStatAuras)?playerStatAuras:[];state.hardDepthRewards=hardDepthRewards;state.weatherEffects=weatherEffects||{};state.abilities=Object.assign({},...abilitySets);state.thumbs=thumbs||{};const reportSnapshot=await restoreReportSnapshot();if(!reportSnapshot){restore();if(visibleUrl.searchParams.get('view')==='tower')state.view='tower'}startWorker();startTowerWorker();render();liveThumbs();if(reportSnapshot){const x=Number(reportSnapshot.viewport?.scrollX)||0,y=Number(reportSnapshot.viewport?.scrollY)||0;setTimeout(()=>window.scrollTo(x,y),80);setTimeout(()=>window.scrollTo(x,y),700)}}catch(err){console.error(err);root.innerHTML=`<div class="error-box"><b>Calculator failed to load.</b><br><br>${esc(err.message)}</div>`}}
  load();
})();
