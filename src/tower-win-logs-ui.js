(()=>{
  ...
  async function openBattle(run){
    ...
    dialog.innerHTML=`<div class="tower-winlog-shell"><header><div><span>INDIVIDUAL WINNING BATTLE</span><b>Battle #${full(run.run)}</b><small>Run ${full(run.run)} of ${full(latestResult?.runs||0)} · ${full(run.turns)} turns · seed ${esc(run.seed||0)}</small></div><div><button type="button" data-prev ${index<=0?'disabled':''}>← Previous</button><button type="button" data-next ${index<0||index>=orderedRuns.length-1?'disabled':''}>Next →</button><button type="button" data-close>Close</button></div></header><main class="tower-winlog-loading"><div class="tower-winlog-spinner"></div><b>Replaying exact seed…</b><small>Full debug is generated only for this battle.</small></main></div>`;
  ...
  function compactBattleList(runs){
    return runs.map((run)=>`Battle #${run.run} · seed ${run.seed} · ${run.turns} turns · survivors ${(run.survivingAllies||[]).join(' / ')||'none'}`).join('\n');
  }
